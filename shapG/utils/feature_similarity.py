            ...
            if type_i == 'numerical' and type_j == 'numerical':
                ...
                try:
                    ...
                    try:
                        ...
                        h_i = -np.sum([p * np.log(p + 1e-10) for p in np.bincount(x_i.astype(int)) / len(x_i) if p > 0])
                        h_j = -np.sum([p * np.log(p + 1e-10) for p in np.bincount(x_j.astype(int)) / len(x_j) if p > 0])
                ...
                try:
                    ...
                    if type_i == 'numerical':
                        x_i_discrete = pd.qcut(x_i, q=5, labels=False, duplicates='drop')
                    else:
                        x_i_discrete = x_i
                        
                    if type_j == 'numerical':
                        x_j_discrete = pd.qcut(x_j, q=5, labels=False, duplicates='drop')
                    else:
                        x_j_discrete = x_j
...
def get_feature_ranking_mixed(X, y, feature_types=None, feature_ranges=None):
    ...
    for i in range(n_features):
        ...
        try:
            if feature_type == 'numerical' and y_type in ['numerical', 'binary']:
                ...
                max_mi = min(np.log(len(np.unique(x_discrete))), np.log(len(np.unique(y_discrete))))
    ...
    sorted_indices = sorted(importance_scores.keys(), key=lambda k: importance_scores[k])
