...
class TestShapleyValue(unittest.TestCase):
    ...
    def test_02_shapley_value(self):
        ...
        self.assertEqual(
            len(TestShapleyValue.shapley_values), TestShapleyValue.G.number_of_nodes()
        )
        ...
        full_coalition_value = coalition_degree(
            TestShapleyValue.G, set(TestShapleyValue.G.nodes())
        )
    ...
    def test_03_shapG(self):
        ...
        self.assertEqual(
            len(TestShapleyValue.shapley_values_G), TestShapleyValue.G.number_of_nodes()
        )
    ...
    def test_04_compare_results(self):
        ...
        for node in sorted(exact.keys()):
            ...
            table_data.append(
                [node, f"{sv:.4f}", f"{asv:.4f}", f"{diff:.2e}", rel_diff]
            )
        ...
        self.assertGreater(
            correlation,
            0.8,
            "Correlation between exact and approximate values is too low",
        )
